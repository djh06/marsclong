...
static const long kMaxLogFileSzie = 3*1024;
static const long KTickSecond = 5*60*1000;
...
static void __make_uploadfilename(const timeval& _tv, const std::string& _logdir, char* _filepath,int _len){
    time_t usec = _tv.tv_usec;
    std::string logfilepath = _logdir;
    logfilepath += "/uploadfile";
    char temp [64] = {0};
    snprintf(temp, 64, "_%ld",usec);
    logfilepath += temp;
    logfilepath += LOG_EXTENSION;
...
static bool __push_logfile2vec(std::string filepath){
    ...
    for (iter = sg_uploadfiles.begin(); iter != sg_uploadfiles.end() ; iter++) {
        std::string tmppath = *iter;
        if (tmppath == filepath) {
            pathexist = true;
        }
    }
    
    if (!pathexist && filepath.length() > 10) {
        printf("filename = >> %s",filepath.c_str());
        sg_uploadfiles.insert(sg_uploadfiles.begin(), filepath);
        return true;
    }
    return false;
}
    ...
    if (boost::filesystem::exists(path) && boost::filesystem::is_directory(path)){
        ...
        for (boost::filesystem::directory_iterator iter(path); iter != end_iter; ++iter) {
            ...
            if (now_time > fileModifyTime && now_time - fileModifyTime > kMaxLogAliveTime) {
                if (boost::filesystem::is_regular_file(iter->status())) {
                    if (iter->path().extension().string() == LOG_EXTENSION) {
                        // todo upload log file and del this file
                        if (__push_logfile2vec(iter->path().string())) {
...
static void __del_timeout_timifile(){
    time_t now_time = time(NULL);
    boost::filesystem::path path(sg_current_dir);
...
static void __upload_logfile(){
    while (sg_uploadfiles.size() > 0) {
...
static bool __openlogfile(const std::string& _log_dir) {
    ...
    long unsigned int fsize =  boost::filesystem::file_size(logfilepath);
    
    if (fsize > kMaxLogFileSzie) {
...
static void __async_curl_thread(){
    while (true) {
        ...
        sg_cond_curl_async.wait(15*60*1000);
    }
}
