...
void log_formater_json(const XLoggerInfo* _info, const XLoggerAppInfo* _app ,const char* _logbody, PtrBuffer& _log) {
    ...
    if (_log.MaxLength() <= _log.Length() + 5 * 1024) {  // allowd len(_log) <= 11K(16K - 5K)
        ...
        if (_log.MaxLength() >= _log.Length() + 128) {
            int ret = snprintf((char*)_log.PosPtr(), 1024, "{\"LogType\":\"F\",\"content\":\"this log is error\"}\n");  // **CPPLINT SKIP**
    ...
    if (NULL != _info) {
        ...
        int ret = snprintf((char*)_log.PosPtr(), 1024, "{\"logtype\":\"%s\",\"content\":\"%s\"}\n",  // **CPPLINT SKIP**
                            levelStrings[kLevelVerbose],_logbody ? _logbody : "error log");
        assert(0 <= ret);
        _log.Length(_log.Pos() + ret, _log.Length() + ret);
        //      memcpy((char*)_log.PosPtr() + 1, "\0", 1);
