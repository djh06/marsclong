...
void xlogger_Write(const XLoggerInfo* _info, const char* _log) {
     ...
     *  B->()
     */
    TLogLevel collectleve = xlogger_Level();
    if (NULL != &__xlogger_Write_impl && _info){
        switch (collectleve) {
            case kLevelDebug:
                break;
            case kLevelFatal:
            {
                if (_info->level == kLevelDebug ||
                    _info->level == kLevelAll ||
                    _info->level == kLevelVerbose) {
                    return;
                }
            }break;
